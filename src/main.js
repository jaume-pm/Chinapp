...
(async function initApp() {
  try {
    ...
    // Fetch Data
    const [hsk1Res, hsk2Res] = await Promise.all([
      fetch('./src/data/hsk1.json'),
      fetch('./src/data/hsk2.json')
    ]);

    if (!hsk1Res.ok) throw new Error(`Failed to load HSK1 data: ${hsk1Res.statusText}`);
    if (!hsk2Res.ok) throw new Error(`Failed to load HSK2 data: ${hsk2Res.statusText}`);

    const hsk1Data = await hsk1Res.json();
    const hsk2Data = await hsk2Res.json();
  ...
  `;
    ...
    function renderLesson() {
      ...
      const pdfUrl = `./public/pdfs/${lesson.level}/${lesson.resource.pdf_file}`;
